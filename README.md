...
### 4. 标注模型显存与内存双保险释放机制
数据标注时加载的深度学习模型（YOLO-seg、YOLO-World、SAM 等）具备**高实时性与安全兜底双保险释放机制**：
* **实时释放（页面关闭即卸载）**：前端通过现代浏览器的 `navigator.sendBeacon` 与 `pagehide` 生命周期，在用户关闭标签页、关闭浏览器或离开页面时，即时异步向后端 `/api/labeling/unload_models` 发送卸载指令，无需等待即刻回收 GPU 显存与系统内存；
* **空闲超时兜底（5 分钟未推理自动卸载）**：若用户保持页面打开但超过 300 秒（5 分钟）未触发任何推理操作，后台自动定时扫描协程（每 60 秒轮询）将触发深度休眠卸载，确保显存不会被长期无意义占驻。
